    ...
    <div className="space-y-8">
      {blocks.map((block, i) => {
        switch (block.type) {
          ...
          case "heading":
            return (
              <h2 key={i} className="pt-2 text-2xl font-semibold tracking-tight text-navy">
                {block.text}
              </h2>
            );
          case "cards":
          ...
          case "ideaOutputs":
            return (
              <div key={i} className="glass-panel p-5 sm:p-6">
                <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-accent">Core idea</p>
                <p className="mt-2 text-lg font-semibold text-navy">&ldquo;{block.core}&rdquo;</p>
                <div className="mt-5 grid gap-3 sm:grid-cols-2">
                  {block.outputs.map((output) => (
                    <div key={output.channel} className="rounded-xl border border-border-subtle bg-surface-muted/40 p-3">
                      <span className="text-[11px] font-semibold uppercase tracking-wide text-primary">
                        {output.channel}
                      </span>
                      <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">{output.format}</p>
                    </div>
                  ))}
                </div>
              </div>
